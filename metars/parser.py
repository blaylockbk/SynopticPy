...
def metar_parser(metar, dt=None):
    ...
    try:
        wind_dir = int(wind[0])
        wind_speed = int(wind[1])
        components = mpcalc.wind_components(wind_speed*units.kt, wind_dir*units.degrees)
    ...
    try:
        ...
        cloud_code_mapping = dict(CLR=0., FEW=25., SCT=50., BKN=75., OVC=100., VV=100.)
    ...
    # Present weather
    wx_re = re.compile('\s(TSNO|VA|FU|HZ|DU|BLDU|SA|BLSA|VCBLSAVCBLDU|BLPY|PO|VCPO|VCDS|VCSS|BR|BCBR|BC|MIFG|VCTS|VIRGA \
    ...
    try:
        ...
        temp = ((-float(group[0][1:]) * units.degC if group[0][0] == 'M' else float(group[0]) * units.degC).to('degF')).magnitude
        dew = ((-float(group[1][1:]) * units.degC if group[1][0] == 'M' else float(group[1]) * units.degC).to('degF')).magnitude
    except:
        temp = np.nan
        dew = np.nan
    try:
        ...
        temp = ((-float(group[0][1:])/10 * units.degC if group[0][0] == '1' else float(group[0])/10 *units.degC).to('degF')).magnitude
        dew = ((-float(group[1][1:])/10 * units.degC if group[1][0] == '1' else float(group[1])/10 * units.degC).to('degF')).magnitude
    except:
        None
    ...
    try:
        ...
        temp_6hrmax = round(((-float(group[0][1:])/10 * units.degC if group[0][0] == '1' else float(group[0])/10 * units.degC).to('degF')).magnitude)
        temp_6hrmin = round(((-float(group[1][1:])/10 * units.degC if group[1][0] == '1' else float(group[1])/10 * units.degC).to('degF')).magnitude)
    except:
        temp_6hrmax = np.nan
        temp_6hrmin = np.nan
    try:
        group = re.findall("\s4(\d{4})(\d{4})", metar)[0]
        temp_24hrmax = round(((-float(group[0][1:])/10 * units.degC if group[0][0] == '1' else float(group[0])/10 * units.degC).to('degF')).magnitude)
        temp_24hrmin = round(((-float(group[1][1:])/10 * units.degC if group[1][0] == '1' else float(group[1])/10 * units.degC).to('degF')).magnitude)
    except:
        temp_24hrmax = np.nan
        temp_24hrmin = np.nan

    return dict(stid=stid, time=time, temp_f=temp, dew_f=dew, wx=wx, wx2=wx2, wx3=wx3,
                wind_dir_deg=wind_dir, wind_speed_kt=wind_speed, wind_gust_kt=wind_gust, u_wind_kt=u_wind, v_wind_kt=v_wind,
                visibility_mi=visibility, cloud_total_pct=cloud_total, cloud_base_ft=cloud_base,
                alti_inhg=alti, slp_mb=slp, temp_6hrmax_f=temp_6hrmax, temp_6hrmin_f=temp_6hrmin,
                temp_24hrmax_f=temp_24hrmax, temp_24hrmin_f=temp_24hrmin,
                precip_1hr_in=precip_1hr, precip_3hr_in=precip_3hr, precip_6hr_in=precip_6hr, precip_24hr_in=precip_24hr,
                snowd_in=snow_d, metar=metar)
...
def station_data_parser(stndata):
    ...
    for stn in stndata:
        try:
            ...
            direct = stn['OBSERVATIONS']['wind_direction_value_1']['value'] * units('degree')
            u, v = mpcalc.wind_components(speed, direct)
            stn = dict(stid=stn['STID'], lat=float(stn['LATITUDE']), lon=float(stn['LONGITUDE']),
                       temp=stn['OBSERVATIONS']['air_temp_value_1']['value'] or np.nan,
                       dew=stn['OBSERVATIONS']['dew_point_temperature_value_1']['value'] or np.nan,
                       slp=stn['OBSERVATIONS']['sea_level_pressure_value_1d']['value'] or np.nan,
                       u=u.magnitude, v=v.magnitude)
            data.append(stn)
        except KeyError:
            continue
